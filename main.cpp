...
int main() {
  ...
  int clientfd = accept(server_sock.fd(), client_addr.addr(), &client_addr_len);
  ...
  int read_bytes = read(client_sock.fd(), read_buffer, BUFFER_MAX_SIZE);
  if (read_bytes == -1) {
  ...
  std::string write_buffer =
      ...
      "OK";
  int write_bytes =
      write(client_sock.fd(), write_buffer.c_str(), write_buffer.size());
  if (write_bytes == -1) {
